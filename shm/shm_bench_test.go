...
func benchmarkAllocateAndDestroy(size uint32, b *testing.B) {
	for n := 0; n < b.N; n++ {
...
// Full Read: ioutil
func benchmarkReadFullAuto(size uint32, b *testing.B) {
	segment, _ := Create(size)
	segmentId = segment.Id
...
func benchmarkReadFullPreallocate(size uint32, b *testing.B) {
	segment, _ := Create(size)
	segmentId = segment.Id
	data = make([]byte, size)
...
// Full Read: Preallocated Slice
func benchmarkReadChunkFull(size uint32, b *testing.B) {
	segment, _ := Create(size)
	segmentId = segment.Id
	var data []byte
